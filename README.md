...
| 6 | 向量化访存 | 18237 | 待测 | ? | ✅ |
| 9 | Autotuning | 19721 | 待测 | ? | ✅ 本课 |
| 10 | Warptiling | 21779 | 待测 | ? | ✅ 本课 |
| 9 | Autotuning | 19721 | | | ⬜ |
| 10 | Warptiling | 21779 | | | ⬜ |
...
### Lesson 9 — Autotuning(参数搜索)

**代码**:[kernels/09_autotuned.cuh](kernels/09_autotuned.cuh)

**痛点**:k5/k6 的 (BM,BN,BK,TM,TN) = (128,128,8,8,8) 是文章按 A6000 逐步调出来的,为什么不是别的?"靠感觉"在 k4 和 k5 之间猜到 9215→15971 的 1.88×。

**手法**:把参数当模板输入,写脚本扫 (BM,BN,BK,TM,TM) 所有可能组合,每个跑一遍、取 GFLOPs/s 最大那组。文章扫了 ~200 组:A6000 上最终选 (128,128,16,8,8) 得 19721 GFLOPs/s。

**和 k6 的区别**:k9 的 BK 从 8 变 16(SMEM tile 深度翻倍),意味着同一块 SMEM 上能多加 1 次乘法周期;SMEM 用满提前(128×16×4 = 8KB Bs,接近 16KB/128 块的 50% 快满)。线程数还是 256(128×128/(8×8)),只是分工微调。

**关键观察**:
- 分数最高的组合 (128,128,16,8,8),而非 (128,256,16,8,8)——BN 再大,mem-bound 反而受 SMB lock。
- BK=16 比 BK=8 好 ~6%:加载指令减半,但 SMEM 读和 FFMA 周期匹配得更好。
- 没有一触即发的定理,只能扫。

**A100 实测**:待填。(128,128,16,8,8) 是文章 A6000 的局部最优,A100 FP32 peak 高一筹、L2 更大,最优参数可能偏 (64,256,16,8,8) 或 (128,128,32,8,8)。需要扫一下。

### Lesson 10 — Warptiling(warp 级分工)

**代码**:[kernels/10_warptiling.cuh](kernels/10_warptiling.cuh)

**痛点**:k9 里 256 线程铺 16×16 网格,每个线程独立读 SMEM、独立算 64 个 FFMA。没有 warp 级的 SMEM 共享。FFMA 读两条 SMEM load 的指令延迟 30 周期里,没有其他 warp 填充 SM。

**手法**:第三级分块——warp 也得到自己的子格。模板六个数:(BM,BN,BK,WM,WN,WMITER,WNITER,TM,TN)。
- BM×BN = 128×128(块级 tile,不变)
- BM/WM × BN/WN = 128/64 × 128/64 = 2×2 = 4 个 warptile,每 warp 管一个 64×64 warptile;
- WMITER × WNITER = 2×2 个 warptile 子块,每个子块 32×32;
- 线程级 TM=8、TN=4,外积 4×8=32 次 FFMA。每 warp 一次迭代 32×32 = 1024 次计算。

**额外招**:As 在加载时转置存储(As[col*BM+row]),计算时 warp 内连续 threadIdx 读连续 SMEM 地址,bank conflict 几乎归零(NCU vLM conflicts 从 3.1% 降到 0%)。

**代价**:~90 寄存器/线程 × 128 线程 = 11520 个寄存器(一个 SM 65536),每 SM 只驻留 1 个 block。值得这个代价:每个 warp 专管 64×64 warptile,SMEM 读到 FFMA 的延迟用 1024 个 FFMA 来摊,而 cuBLAS 还得粗分块。`__launch_bounds__(NUM_THREADS, 1)` 控制寄存器不溢出、occupancy 明确为 1。

**A100 实测**:待填。文章 A6000 是 k9 的 1.10×(19721→21779)。(128,128,16,4,4,2,8,4) 是 A6000 局部最优,A100 FP32 峰值更低(一半),寄存器压力相对更大,最优可能是 (128,128,16,4,4,1,8,8)(WMITER=1,寄存器减半)。需扫。
